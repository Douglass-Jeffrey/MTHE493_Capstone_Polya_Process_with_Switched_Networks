...
SCENARIOS = 10
POLY_STEPS = 1000
awarness = np.zeros((SCENARIOS, POLY_STEPS))

for s in range(0, SCENARIOS):
    num_colors = 2
    initial_nodes = 8
    num_batches = cp_module.array([1024, 512, 256], dtype=cp_module.int32)   #multiply and add these arrays to get total numebr of users
    batch_sizes = cp_module.array([   2,  32, 512], dtype=cp_module.int32)
    barabasi_num_connections = 5 #num conections for a new node

    num_interventions = 120 #num intervention nodes added 
    per_intervention_num_connections = 1000 #num of  new connections to each intervention node
    intervention_deltas=1 #does nothing
    intervener_urn = cp_module.array([(s+1)*10,0], dtype=cp_module.int32)
    polya_steps = POLY_STEPS
...
time = np.arange(POLY_STEPS)

#print("time shape:", time.shape)
#print("awarness shape:", awarness.shape)

for s in range(SCENARIOS):
    plt.plot(time, awarness[s, :], label=f"Initial Red {(s+1)*10}")
    print(f"Scenario {(s+1)*10} final awareness: {awarness[s, -1]:.4f}")

plt.xlabel("Time")
plt.ylabel("Proportional Aware")
plt.title("Awarness vs Initial Red Concentration")
plt.legend()
plt.show()
