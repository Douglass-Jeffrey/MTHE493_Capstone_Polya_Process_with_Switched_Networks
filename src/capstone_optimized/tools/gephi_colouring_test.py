...
num_colors = 2
initial_nodes = 64
num_batches = cp_module.array([65536,], dtype=cp_module.int32) #128
batch_sizes = cp_module.array([   1,], dtype=cp_module.int32) #65536
barabasi_num_connections = 1

num_interventions = 1
per_intervention_num_injections = 2048
intervener_urn = cp_module.array([148,0], dtype=cp_module.int32)
...
colours = np.empty((graph.num_nodes, 2))

num_bins = 72
hist_bins = (cp_module.linspace(0, 1, num_bins+1))
hist_data = (cp_module.zeros((polya_steps, num_bins)))
for step_i in range(POLYA_STEPS):
    ...
    #collect data
    colours = ((graph.node_urns[:, 0] / cp_module.sum(graph.node_urns, axis=1)))
    colours_b = np.where(colours > 0.80, 'red', 'black')
    nodes_colours = np.column_stack([np.arange(graph.num_nodes), colours_b])
    np.savetxt(f"colours_{step_i+1}.csv", nodes_colours, delimiter=",", fmt="%s")
    #collect data
    hist_i, _ = cp_module.histogram(colours, hist_bins)
    hist_data[step_i] = hist_i
...
csv_path = "hist_data_gephi.csv"
np.savetxt(csv_path, hist_data, delimiter=",", fmt="%d")
print(f"\nsaved gephi hist_data to {csv_path}")
